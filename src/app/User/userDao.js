/**
// 유저 생성
async function createUserInfo(connection, userInfoParams) {
    const userInfoQuery = `
          INSERT INTO User(email, password, birth, gender, nickname, notes)
          VALUES (?, ?, ?, ?, ?, ?);
      `;
    const userInfoRow = await connection.query(userInfoQuery, userInfoParams);
